...
!!! warning "The old flat imports were removed in 0.6.0"
    `from param_id.paramID import CVS0DParamID` (no `libcuflynx.` prefix) resolved in
    0.4.0 and 0.5.x as a shim that warned. In 0.6.0 it raises `ModuleNotFoundError`;
    prefix the import with `libcuflynx.`. See `CHANGELOG.md`.
