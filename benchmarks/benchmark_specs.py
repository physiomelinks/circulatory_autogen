...
def run_three_compartment(base_config, resources_dir, output_dir, generated_models_dir,
    ...
    result = BenchmarkResult(
        ...
        description=('Gradient-free global searches vs multi-start L-BFGS-B with the two '
                     ...
                     'is the max RELATIVE deviation, since these parameters span five orders of '
                     'magnitude.'),
    ...
    if rank == 0:
        for method in methods:
            ...
            result.rows.append(BenchmarkRow(
                ...
                cost=float(comparison.results[method]['cost']),
                time_s=float(comparison.runtimes[method]),
                # RELATIVE error: these four parameters span ~8e-4 to ~3.7e8, so a max absolute
                # deviation would be entirely dominated by E_lv_A and say nothing about the
                # others. (The other benchmarks' parameters are same-order, so they report
                # absolute error -- see each benchmark's description.)
                param_err=float(np.max(np.abs(
                    (params - THREE_COMPARTMENT_TRUE_PARAMS) / THREE_COMPARTMENT_TRUE_PARAMS))),
                params=[float(p) for p in params]))
