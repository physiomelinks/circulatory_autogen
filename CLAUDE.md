...
- **Releasing**: `CONTRIBUTING.md` ("Making a release"). Cut from a `v*` tag; `.github/workflows/release.yml` builds wheel + sdist and publishes to PyPI by trusted publishing (OIDC), in the environment `pypi` — the workflow filename and that environment name are bound to PyPI's registered publisher, so renaming either breaks publishing. The tag must match `version` in `pyproject.toml` (CI checks), and a PyPI version can never be re-uploaded — a bad release means a new version, not a fixed one.
