...
- `model_type` — `cellml_only` (default) | `python` | `casadi_python` | `cpp` | `aadc_python` | `python_user_defined` | `external_python`.
- `solver` — `CVODE_myokit` (default) | `CVODE_opencor` | `solve_ivp` (python models) | `casadi_integrator` (casadi_python models) | `RK4_cpp` | `user_defined` (python_user_defined) | `external` (external_python).
...
- **The two user-supplied model types.** `python_user_defined` takes an **rhs** and CA integrates it with scipy `solve_ivp` (`model_wrapper_path`, default `funcs_user/{prefix}_wrapper.py`; example in `funcs_user/example_model/`). `external_python` takes a **solver class** that does its own time stepping — an FE/FV code, a compiled library — and CA only wraps it (`external_model_path`, default `funcs_user/{prefix}_model.py`; example in `funcs_user/example_model_external/`). The two path keys are deliberately separate, since the files satisfy different contracts. For `external_python` the file must define `SIM_HELPER = YourClass`, and the class must declare literal `parameters` / `output_names` attributes (read by AST, without importing) plus `init_solver` / `update_times` / `set_param_vals` / `run` / `get_results`; `get_init_param_vals` / `reset` / `extra_plots` / `close` are optional. Its only solver_info setting is `user_config`, a free-form dict handed to `init_solver`. Neither type has a generation step. See `src/solver_wrappers/external_simulation_helper.py` for the full contract.
...
| `test_python_user_defined.py`, `test_external_simulation_helper.py` | The two user-supplied model types, end to end (`unit` for the contract checks; `integration`/`slow`/`mpi` for calibration, SA and local sensitivities) |
