...
| `test_external_simulation_helper.py`, `test_scipy_ode_example.py` | The user-supplied model type, end to end (`unit` for the contract checks; `integration`/`slow`/`mpi` for calibration, SA and local sensitivities). The scipy file also pins the refusal of the removed `python_user_defined` config. |
