...
User-extensible (kept outside `src/`): `module_config_user/` (custom CellML modules), `funcs_user/` (your own cost / operation / modifier funcs, plus the `external_python` model examples — see `funcs_user/README.md`). `resources/` holds input CSVs and obs_data.json; `generated_models/` is build output.
