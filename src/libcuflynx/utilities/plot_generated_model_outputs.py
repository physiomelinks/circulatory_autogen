...
import numpy as np
import os
from libcuflynx.solver_wrappers.opencor_helper import SimulationHelper
from libcuflynx.utilities import paperPlotSetup
paperPlotSetup.Setup_Plot(3)
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from libcuflynx.utilities.paths import default_generated_models_dir


def main():
    """Example: sweep two parameters of the 3compartment model and plot the outputs.

    Everything here used to run at *import* time, including an os.makedirs -- so merely
    importing this module did a simulation and wrote files. Under a main guard now, and
    the paths come from generated_models_dir rather than from a repo root guessed off
    __file__ (#431).
    """
    ...
    file_path = os.path.join(generated_models_dir, '3compartment', '3compartment.cellml')
    output_file_path = os.path.join(generated_models_dir, '3compartment', 'generated_outputs')
